# Sieger pixIQ — Yarn Cone Inspection System (Jetson Orin NX)

Industrial yarn cone quality inspection system for GHCL textile mills, deployed on **NVIDIA Jetson Orin NX 16GB** (pixIQ edge appliance). Inspects yarn cones in real time using three Basler GigE cameras, communicating pass/fail results to a Siemens PLC over Modbus TCP.

> This is the **Jetson Orin NX / Basler** variant. For the x86 + IDS cameras variant, see the `cone-transport-system` repo.
...
| Component | Spec |
|-----------|------|
| Host | **NVIDIA Jetson Orin NX 16GB** (ARM64, JetPack 6.x) |
| OS | Ubuntu 22.04 LTS (L4T) |
| Inference | TensorRT FP16 (auto-built via `scripts/export_tensorrt.py`), PyTorch FP16 fallback |
| Cameras | 3× **Basler GigE** — VL: a2A2600-20gcPRO (2600×2048, 25mm), UV: acA1920-40gc (16mm), Tail: a2A1920-40gc (25mm); identified by static IP |
| Camera SDK | Basler pylon + `pypylon` (ARM64: install pylon SDK first, then `pip install pypylon --no-binary pypylon`) |
| Trigger | Hardware trigger on **Line1**, `GrabStrategy_LatestImageOnly` |
| PLC | Siemens S7, Modbus TCP at 192.168.2.61:502 |
| HMI | Separate all-in-one touchscreen desktop (not on Jetson) |

> **Memory budget:** 16GB shared CPU+GPU — see `docs/project_context.md` for the per-module budget.
...
│   ├── camera/                  # Basler pylon (pypylon) wrappers
